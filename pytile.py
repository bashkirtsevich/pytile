...
class TrackSprite(pygame.sprite.Sprite):
    ...
    def __init__(self, xWorld, yWorld, zWorld, init_paths=None,
        ...
        if TrackSprite.init:
            ...
            tex = pygame.image.load("road_bg.png")
    ...
    def draw_sleepers(self, control_points):
        ...
        for p in range(1, len(cps)):
            ...
            for n in range(s, n_sleepers + 1):
                sleep_p = [self.bezier.get_at_width(
                    a - start_vector + n * ab_n * true_spacing - ab_n * 0.5 * self.sleeper_width, a_to_b,
                    -self.sleeper_length),
                    self.bezier.get_at_width(
                        a - start_vector + n * ab_n * true_spacing - ab_n * 0.5 * self.sleeper_width, a_to_b,
                        self.sleeper_length),
                    self.bezier.get_at_width(
                        a - start_vector + n * ab_n * true_spacing + ab_n * 0.5 * self.sleeper_width, a_to_b,
